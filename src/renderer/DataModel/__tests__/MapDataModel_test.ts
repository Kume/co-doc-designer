...
describe('Test for valueForKey', () => {
  it('Can get value', () => {
    ...
    expect(model.valueForKey('a')).toEqual(new ScalarDataModel(2));
  });
...
describe('Test for getValue', () => {
  ...
  it('Can get value deeply', () => {
    ...
    expect(model.getValue(path)).toEqual(new ScalarDataModel(9));
  });
});
...
describe('Test for setValue', () => {
  it('Can set value for key', () => {
    const model = new MapDataModel({a: 2});
    const path = new DataPath('a');
    const updatedModel = model.setValue(path, new ScalarDataModel(5)) as MapDataModel;
    expect(updatedModel.valueForKey('a')).toEqual(new ScalarDataModel(5));
  });

  it('Can set value for key deeply', () => {
    const model = new MapDataModel({a: {b: {c: 9}}});
    const path = new DataPath(['a', 'b']);
    const updatedModel = model.setValue(path, new ScalarDataModel(5));
