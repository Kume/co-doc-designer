...
describe('Unit tests for DataPath', () => {
  describe('Unit tests for DataPath.parse', () => {
    ...
    it('Can parse context key included', () => {
      const path = DataPath.parse('..ctx/../a', [{ key: 'ctx', depth: 2}]);
      expect(path.elements.size).toBe(4);
      expect(path.elements.map(e => e.type).toArray()).toEqual([
        DataPathElement.Type.Reverse,
        DataPathElement.Type.Reverse,
        DataPathElement.Type.Reverse,
        DataPathElement.Type.MapKey,
      ]);
    });
  });
